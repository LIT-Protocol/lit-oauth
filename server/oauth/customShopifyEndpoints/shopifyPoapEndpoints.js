...
const validatePoapToken = async (token) => {
  const removeBearer = token.split(' ');
  const splitToken = removeBearer[1];
  return new Promise((resolve, reject) => {
    jsonwebtoken.verify(splitToken, process.env.LIT_POAP_SHOPIFY_SECRET, ['H256'], (err, decoded) => {
      if (err) reject(false);
...
export default async function shopifyPoapEndpoints(fastify, opts) {
  ...
  fastify.post("/api/shopify/checkIfPoapProductHasBeenUsed", async (request, reply) => {
      try {
        const result = await validatePoapToken(
  ...
  fastify.post("/api/shopify/deletePoapDraftOrder", async (request, reply) => {
    const result = await validatePoapToken(request.headers.authorization);
