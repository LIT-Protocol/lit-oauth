...
export default async function (fastify, opts) {
  fastify.post("/api/shopify/saveDiscount", async (request, reply) => {
    const newSavedDiscount = await fastify.objection.models.shopifyShares
      ...
      .insert({
        ...
        humanized_access_control_conditions:
          request.body.humanized_access_control_conditions,
    ...
    await sendSlackMetricsReportMessage({
      msg: `Shopify discount created for store id ${request.body.store_id}`,
    });
  ...
  fastify.post("/api/shopify/deleteAllDiscounts", async (request, reply) => {
    const deleteAllDiscountsResult =
      await fastify.objection.models.shopifyShares
        .query()
        .delete()
